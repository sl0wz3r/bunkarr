...
// emptied source, dry runs and the mass-change guard. The syncs and kill tests also snapshot the
// source tree around every job: Bunkarr must never modify it (S1). BUNKARR_E2E_BINARY runs another
// binary; BUNKARR_E2E_TARGET_ROOT puts the destinations under that directory (the share test).
//
...
// BUNKARR_E2E_SHARES. They use named volumes and reach every HTTP API through `docker exec`, so
