...
- **Method: SQLite online backup, one step.**
  ...
  - Never use incremental steps, a read-write connection, or `immutable=1` while `<db>-wal`
    exists.
...
- **Staging.**
  - The backup API writes to local staging, `/config/staging/plexdb-<jobID>/`, never directly
    over SMB/NFS: temp name, fsync, rename.
  - Verify the file there. The `filecopy` engine then copies it to
    `<target>/.bunkarr/plex/<slug>/<ts>/` with size and sha256 checks (S7), next to
    `manifest.json`. Then remove the staging directory.
...
- **Verification (`plexdb.Verify`).**
  ...
  - Register a binary-compare stub with `sqlite.RegisterCollationUtf8` for every non-built-in
    collation named in `sqlite_master`. Today that is only `icu_root`. Register each name once
    per process.
...
- **Tests.** The slow Docker suite (acceptance 4) does the following:
  - pins `plexinc/pms-docker` by digest and enables API access with an explicit-subnet
    `allowedNetworks`;
...
- `RegisterCollationUtf8` is process-wide. Bunkarr's own schema never uses Plex's collation
  names, so the stub cannot affect it.
